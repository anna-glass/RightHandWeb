  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      const { error: signInError } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${window.location.origin}/signin?login=true`,
          queryParams: {
        ...
        <div className="space-y-6">
          ...
          <div className="flex justify-center">
            <button
              ...
              disabled={loading}
              className="flex items-center gap-3 px-6 py-3 bg-white rounded-lg hover:bg-gray-50 hover:scale-[1.02] transition-all disabled:cursor-not-allowed"
            >
